...
def extract_revision_data(element):
    ...
    return {"change_number": change_number, "total_revisions": total_revisions, "branch": branch}
...
def calculate_time_difference(start, end):
    if start and end:
        ...
        return round(diff_datetime.total_seconds() / 60, 2)
    else:
        return None, None

####### Main body
list_changes = "https://gerrit-review.googlesource.com/changes/?q=project:gerrit+AND+not+dir:polygerrit-ui+AND+(branch:stable-3.7+OR+branch:stable-3.8+OR+branch:stable-3.9+OR+branch:master)+AND+-age:9week&o=CURRENT_REVISION"
    ...
    for change in changes:
        ...
        for revision_number in range(1, total_revisions + 1):
            ...
            if any(check['checker_name'] == 'RBE BB Build/Tests' for check in checks_json_data):
                print(f"Processing change: {change_number} and revision: {revision_number} and branch {branch} and total revisions {total_revisions}")

                rbe_gcp_started, rbe_gcp_state, rbe_gcp_start, rbe_gcp_end, rbe_bb_started, rbe_bb_state, rbe_bb_start, rbe_bb_end = process_checks_data(checks_json_data)

                gcp_time_difference_minutes = calculate_time_difference(rbe_gcp_start, rbe_gcp_end)
                bb_time_difference_minutes = calculate_time_difference(rbe_bb_start, rbe_bb_end)
                ...
                changes_revisions.append({
                    ...
                    "branch": branch,
                    "rbe_gcp_started": rbe_gcp_started,
                    "rbe_gcp_state": rbe_gcp_state,
                    "rbe_gcp_time_minutes": gcp_time_difference_minutes,
                    "rbe_bb_started": rbe_bb_started,
                    "rbe_bb_state": rbe_bb_state,
                    "rbe_bb_time_minutes": bb_time_difference_minutes,
                })
            else:
                print(f"No data for change: {change_number} and revision: {revision_number} and branch {branch} and total revisions {total_revisions}")
