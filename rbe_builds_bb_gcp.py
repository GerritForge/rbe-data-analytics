...
####### Main body
list_changes = "https://gerrit-review.googlesource.com/changes/?q=project:gerrit+AND+not+dir:polygerrit-ui+AND+(branch:stable-3.7+OR+branch:stable-3.8+OR+branch:stable-3.9+OR+branch:master)+AND+-age:1week&o=CURRENT_REVISION"
    ...
    for change in changes:
        ...
        for revision_number in range(1, total_revisions + 1):
            ...
            if any(check['checker_name'] == 'RBE BB Build/Tests' for check in checks_json_data):
                print(f"Processing change: {change_number} and revision: {revision_number} and branch {branch}")
