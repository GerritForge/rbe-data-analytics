...
# Calculate average build times for RBE GCP and RBE BB across revisions
avg_gcp_times = df.groupby('revision_number')['rbe_gcp_time_seconds'].mean()
avg_bb_times = df.groupby('revision_number')['rbe_bb_time_seconds'].mean()
...
plt.title('Average Build Times for RBE GCP and RBE BB Across Revisions')
